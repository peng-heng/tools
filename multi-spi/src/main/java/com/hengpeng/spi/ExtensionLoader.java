...
public final class ExtensionLoader<T> {
    ...
    public List<T> getJoins() {
        ...
        if (Objects.equals(extensionClassesEntity.size(), cachedInstances.size())) {
            return (List<T>) this.cachedInstances.values().stream().sorted(holderComparator).map(e -> {
                return e.getValue();
            }).collect(Collectors.toList());
        }
        List<T> joins = new ArrayList<>();
        List<ClassEntity> classEntities = extensionClassesEntity.values().stream().sorted(classEntityComparator).collect(Collectors.toList());
    ...
    private void loadDirectory(final Map<String, ClassEntity> classes) {
        ...
        try {
            ...
            System.err.println(String.format("load extension class error %s", fileName));
        }
    }
    ...
    private void loadClass(final Map<String, ClassEntity> classes, final String name, final String classPath) throws ClassNotFoundException {
        ...
        if (Objects.isNull(oldClassEntity)) {
            ...
            throw new IllegalStateException("load extension resources error,Duplicate class " + clazz.getName() + " name " + name + " on " + oldClassEntity.getClazz().getName() + " or " + subClass.getName());
        }
    }
